...
public class TimelineView extends View
{
	...
	public static final int DISTANZA_FRA_FATTI = 5;
	...
	private void init()
	{
		...
		TimelineView.x  = 0;
		TimelineView.y = 0;
		TimelineView.ySalvata = 0;
		TimelineView.widthMax = 0;
		TimelineView.heightMax = 0;
		TimelineView.giaDisegnato = false;
	}
	...
	public void setPosizione(int x, int y)
	{
		TimelineView.x = x;
		TimelineView.y = y;
		TimelineView.ySalvata = y;
	}
	
	public void inserisciTimelines(ArrayList<TimelineGrafica> tls)
	{
		listaTl = new ArrayList<TimelineGrafica>(tls);
	...
	{		
		...
		x[0] = t.getUltimoGrain() * t.getDistanzaIntervalli() 
			   + TimelineView.x + 2 * t.getDistanzaIntervalli();
		y[0] = TimelineView.y + SPAZIO_PER_FATTI_SOTTO;
		
		//Punta del triangolo
		x[1] =  t.getUltimoGrain() * t.getDistanzaIntervalli() + 40 + TimelineView.x + 2 * t.getDistanzaIntervalli();
		y[1] = TimelineView.y + (t.getSpessoreTimeline()/2) + SPAZIO_PER_FATTI_SOPRA;
		
		x[2] = t.getUltimoGrain() * t.getDistanzaIntervalli() + TimelineView.x+ 2 * t.getDistanzaIntervalli();
		y[2] = TimelineView.y + t.getSpessoreTimeline() + SPAZIO_PER_FATTI_SOTTO  + SPAZIO_PER_FATTI_SOPRA;
		...
		if(this.xAngoloDestroGiu > TimelineView.widthMax)
			TimelineView.widthMax = this.xAngoloDestroGiu;
		if(this.yAngoloDestroGiu > TimelineView.heightMax)
			TimelineView.heightMax = this.yAngoloDestroGiu;
	}
